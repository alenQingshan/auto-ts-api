...
function useModelClass() {
  ...
  console.log('用户数据:', userData.toJSON());
...
const { usersService } = require('./src-js');
...
app.post('/api/users', async (req, res) => {
  try {
    ...
    res.json({ code: 201, data: newUser });
  } catch (error) {
    res.status(500).json({ code: 500, message: error.message });
  }
});
